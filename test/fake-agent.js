...
process.stdout.write(`${E}?25l` + text + '\r\n');
const started = Date.now();
if (process.stdin.isTTY) process.stdin.setRawMode(true);
let got = '';
const finish = () => {
  fs.writeFileSync(resultFile, JSON.stringify({ scenario, keys: got, ms: Date.now() - started }));
  process.stdout.write(`${E}?25h`);
  process.exit(0);
};
process.stdin.on('data', (d) => { got += d.toString(); setTimeout(finish, 300); });
