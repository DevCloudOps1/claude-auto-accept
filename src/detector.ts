...
export const DEFAULT_DENY_LIST: string[] = [
  ...
  ">\\s*/dev/(sd|nvme|disk)",
  ":\\(\\)\\s*\\{.*\\};\\s*:",
  "\\bdrop\\s+(table|database|schema|collection|view|index|user|role)\\b",
  "\\btruncate\\s+(table\\s+)?[\\w\"`\\[]",
  "\\bdelete\\s+from\\b",
  "\\bupdate\\s+\\S+\\s+set\\b(?!.*\\bwhere\\b)",
  "\\balter\\s+table\\b.*\\bdrop\\b",
  "\\b(dropdb|dropuser)\\b",
  "\\.(dropDatabase|drop)\\(\\s*\\)|\\.(deleteMany|remove)\\(\\s*\\{\\s*\\}\\s*\\)",
  "\\bflush(all|db)\\b",
  "\\bprisma\\s+(migrate\\s+reset|db\\s+push\\b.*--(force-reset|accept-data-loss))",
  "\\bdb:(drop|reset|purge|wipe|schema:load)\\b|\\bmigrate:(fresh|reset|refresh)\\b",
  "\\bmanage\\.py\\s+(flush|reset_db|sqlflush)\\b",
  "\\b(supabase|turso|planetscale|neonctl)\\b.*\\b(reset|delete|destroy)\\b",
  "\\bterraform\\s+(destroy|apply\\b.*-destroy)\\b",
  "\\bkubectl\\s+delete\\b",
  "\\bhelm\\s+(uninstall|delete)\\b",
  "\\baws\\s+s3\\s+(rb\\b|rm\\b.*--recursive)|\\baws\\s+\\w+\\s+delete-",
  "\\bdocker\\s+((system|volume)\\s+prune|volume\\s+rm)\\b",
];
